...
GK_S32 sync_af_calc(GK_U64 u64Data)
{
	...
	if (cnt++ % 30 == 0) {
		...
		for (i = 0; i < 15; i++) {
			for (j = 0; j < 17; j++) {
				u16StatData =
					pstIspFocusSt->stZoneMetrics[i][j].u16h1;
...
static long SampleIst_Ioctl(struct file *file, unsigned int cmd,
			    unsigned long arg)
{
	...
	case SAMPLE_IST_ADD_NODE:
		if (CKFN_ISP_RegisterSyncTask()) {
			CALL_ISP_RegisterSyncTask(ViPipe,
						  &syncNode[node_index]);
		} else {
	...
	case SAMPLE_IST_DEL_NODE:
		if (CKFN_ISP_UnRegisterSyncTask()) {
			if (CALL_ISP_UnRegisterSyncTask(
				    ViPipe, &syncNode[node_index]) ==
			    GK_FAILURE) {
