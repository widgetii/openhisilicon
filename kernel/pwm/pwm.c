...
#define IO_PWMI_ADDRESS(x)                                            \
	((void __iomem *)((unsigned char __iomem *)reg_pwmI_base_va + \
			  ((x) - (PWMI_ADRESS_BASE))))
...
#define PWM_WRITE_REG(Addr, Value) \
	...
	if (enable) {
		...
		case 0:
			...
			PWM_WRITE_REG(
				PWM0_CTRL_REG,
				(1 << 2 |
				 PWM_ENABLE)); // keep the pwm always output;
			break;
...
#endif
{
	...
	case PWM_CMD_WRITE: {
		...
		if (read_user_linear_space_valid(
			    (unsigned char *)(GK_UINTPTR_T)arg,
			    sizeof(PWM_DATA_S)) == 0) {
			osal_printk("pwm address is invalid!\n");
			return -EFAULT;
		}

		ret = copy_from_user(&stPwmData,
				     (PWM_DATA_S __user *)(GK_UINTPTR_T)arg,
				     sizeof(PWM_DATA_S));
		if (ret) {
			return -EFAULT;
		}
...
#ifndef __LITEOS__
#ifdef CONFIG_SNAPSHOT_BOOT
struct media_ops stPwmDrvOps = { .pm_freeze = PWM_freeze,
...
#endif
{
	...
	osal_snprintf(s_stPwmDevice.devfs_name,
		      sizeof(s_stPwmDevice.devfs_name), PWM_DEVNAME);
...
static struct platform_driver pwm_driver = { .probe = pwm_probe,
					     .remove = pwm_remove,
					     .driver = {
						     .name = "pwm",
						     .of_match_table =
